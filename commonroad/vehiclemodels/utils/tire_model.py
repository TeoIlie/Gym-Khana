...
def formula_longitudinal_comb(kappa, alpha, F0_x, p):
    ...
    D_xalpha = F0_x / (
        math.cos(
            C_xalpha
            * math.atan(B_xalpha * S_hxalpha - E_xalpha * (B_xalpha * S_hxalpha - math.atan(B_xalpha * S_hxalpha)))
        )
    )
...
def formula_lateral_comb(kappa, alpha, gamma, mu_y, F_z, F0_y, p):
    ...
    D_ykappa = F0_y / (
        math.cos(
            C_ykappa
            * math.atan(B_ykappa * S_hykappa - E_ykappa * (B_ykappa * S_hykappa - math.atan(B_ykappa * S_hykappa)))
        )
    )
    ...
    return (
        D_ykappa
        * math.cos(
            C_ykappa * math.atan(B_ykappa * kappa_s - E_ykappa * (B_ykappa * kappa_s - math.atan(B_ykappa * kappa_s)))
        )
        + S_vykappa
    )
